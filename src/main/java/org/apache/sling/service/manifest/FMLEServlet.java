...
public class FMLEServlet extends SlingSafeMethodsServlet  {
    ...
    protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            String resourcePath = "/libs/mediacenter/fmle/fmle_profile.xml";
            ...
            try {
                Reader reader = new BufferedReader(new InputStreamReader(inpstr, "UTF-16"));
            ...
            String dynProfile = strProfileTemplate.replace("[url]", uStr("rtmp://" + originServerPath + "/" + originServerLiveApp));
            ...
            dynProfile = dynProfile.replace("[stream]", uStr(streamName));
            dynProfile = dynProfile.replace("[videoDatarate]", uStr(videoDatarate));
            dynProfile = dynProfile.replace("[videoOutputsize]", uStr(videoOutputsize));
            dynProfile = dynProfile.replace("[audioFormat]", uStr(audioFormat));
            dynProfile = dynProfile.replace("[audioDatarate]", uStr(audioDatarate));
