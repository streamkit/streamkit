...
public class Encoder {
    ...
    protected final static Logger log = LoggerFactory.getLogger(MediaProperties.class);
	...
	private static final Pattern BIT_RATE_PATTERN = Pattern.compile(
			"bitrate:\\s+(\\d+)\\s+kb/s", Pattern.CASE_INSENSITIVE);
	 ...
	 *             If a problem occurs calling the underlying ffmpeg executable.
	 */
	private MultimediaInfo getInfo(File source, String type) throws InputFormatException,EncoderException, Exception {
		FFMPEGExecutor ffmpeg = locator.createExecutor();
    ...
    public MultimediaInfo getInfoShort(File source) throws InputFormatException,EncoderException, Exception {
        return getInfo(source, "short");
    }

    public MultimediaInfo getInfoLong(File source) throws InputFormatException,EncoderException, Exception {
        return getInfo(source, null);
    }
	...
	private MultimediaInfo parseMultimediaInfo(File source,
			...
			EncoderException {
		Pattern pInput = Pattern.compile("^\\s*Input #0, (\\w+).+$\\s*",
				Pattern.CASE_INSENSITIVE);
		Pattern pDuration = Pattern.compile(
				"^\\s*Duration: (\\d\\d):(\\d\\d):(\\d\\d)\\.(\\d).*$",
				Pattern.CASE_INSENSITIVE);
		Pattern pStream = Pattern.compile(
				"^\\s*Stream #.*: ((?:Audio)|(?:Video)|(?:Data)): (.*)\\s*$",
				Pattern.CASE_INSENSITIVE);
		...
		try {
			...
			while (true) {
				...
				if (step == 0) {
					...
					Matcher m = pDuration.matcher(line);
					if (m.matches()) {
                    ...
                    Matcher m1 = BIT_RATE_PATTERN.matcher(line);
                    if (m1.find()) {
                        int bitRate = Integer.parseInt(m1.group(1));
                        video.setBitRate(bitRate);
                    }
                    step++;
				} else if (step == 2) {
					Matcher m = pStream.matcher(line);
					if (m.matches()) {
						String type = m.group(1);
						String specs = m.group(2);
						if ("Video".equalsIgnoreCase(type)) {
                            // Frame rate
                            Matcher m1 = FRAME_RATE_PATTERN.matcher(line);
                            if (m1.find()) {
                                try {
                                    float frameRate = Float.parseFloat(m1.group(1));
    ...
    private MultimediaInfo parseMultimediaInfoShort(File source, RBufferedReader reader)
            throws InputFormatException, EncoderException, Exception {
        ...
        // Extract duration
        Pattern pDuration = Pattern.compile("Duration: (\\d\\d):(\\d\\d):(\\d\\d)\\.(\\d).*$", Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
        Matcher mDuration = pDuration.matcher(outputStr);
        if (mDuration.find()) {
            long hours = Integer.parseInt(mDuration.group(1));
            long minutes = Integer.parseInt(mDuration.group(2));
            long seconds = Integer.parseInt(mDuration.group(3));
            long dec = Integer.parseInt(mDuration.group(4));
        ...
        // Extract bitrate
        Matcher mVBitrate = BIT_RATE_PATTERN.matcher(outputStr);
        if (mVBitrate.find()) {
            video.setBitRate(Integer.parseInt(mVBitrate.group(1)));
        }

        // Extract framerate
        Matcher mVFramerate = FRAME_RATE_PATTERN.matcher(outputStr);
        if (mVFramerate.find()) {
            video.setFrameRate(Float.parseFloat(mVFramerate.group(1)));
        }

        // Extract size
        Matcher mVSize = SIZE_PATTERN.matcher(outputStr);
        if (mVSize.find()) {
            int width = Integer.parseInt(mVSize.group(1));
            int height = Integer.parseInt(mVSize.group(2));
	...
	public void encode(File source, File target, EncodingAttributes attributes,
		...
		if (videoAttributes == null) {
			...
			Integer vFrames = videoAttributes.getVframes();
			if (vFrames != null) {
				ffmpeg.addArgument("-vframes");
				ffmpeg.addArgument(String.valueOf(vFrames.intValue()));
			}
